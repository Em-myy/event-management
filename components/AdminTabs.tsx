...
/* ═══════════════════════════════════════════════════════════════
   VENUES TAB
═══════════════════════════════════════════════════════════════ */
function VenuesTab({ initial }) {
  const supabase = createClient();
  const [items,   setItems]   = useState(initial);
  const [editing, setEditing] = useState(null);
  const [loading, setLoading] = useState(false);

  const blank = { name: '', location: '', capacity: '', description: '', is_active: true };
      ...
      if (form.id) {
        const { data } = await supabase
          ...
          .update({
            ...
            capacity: Number(form.capacity), description: form.description,
      ...
      <div className="flex justify-end mb-4">
        <button onClick={() => setEditing(blank)}
          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold
                     text-white rounded-xl shadow-sm hover:opacity-90"
...
/* ═══════════════════════════════════════════════════════════════
   RESOURCES TAB
═══════════════════════════════════════════════════════════════ */
function ResourcesTab({ initial }) {
  const supabase = createClient();
  const [items,   setItems]   = useState(initial);
  const [editing, setEditing] = useState(null);
  const [loading, setLoading] = useState(false);

  const blank = { name: '', description: '', total_quantity: '', condition: 'Good', is_active: true };
    ...
    try {
      if (form.id) {
        ...
        const { data } = await supabase
          ...
          .insert({
            name: form.name, description: form.description,
            total_quantity: Number(form.total_quantity), condition: form.condition,
          })
          .select().single();
        setItems(p => [...p, data]);
      }
              ...
              <tr key={r.id}>
                ...
                <td>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold
                                    ${conditionColor[r.condition] ?? conditionColor.Good}`}>
                    {r.condition}
                  </span>
                </td>
                <td>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-300
                               bg-white text-slate-700 font-semibold outline-none
...
/* ═══════════════════════════════════════════════════════════════
   REUSABLE FORM MODAL
═══════════════════════════════════════════════════════════════ */
function FormModal({ title, fields, data, loading, onSave, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-slide-up">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose}
            className="flex-1 py-2.5 text-sm font-semibold text-slate-700
