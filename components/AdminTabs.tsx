...
export default function AdminTabs({
  ...
  return (
    <div className="w-full">
      ...
      {tab === "invites" && (
        <AdminInvitePanel initialInvites={initialInvites} />
      )}
...
function VenuesTab({ initial }: { initial: Venue[] }) {
  ...
  async function executeDelete() {
    ...
    try {
      ...
      setAlertMsg(
        "Failed to delete venue. It might be in use by an existing booking.",
      );
    } finally {
      setIsDeleting(false);
              ...
              {items.map((v) => (
                <tr key={v.id} className="border-t border-slate-100">
                  <td className="px-4 sm:px-6 py-3 font-medium text-slate-900">
                    {v.name}
                    {v.description && (
                      <div className="text-xs text-slate-400 font-normal mt-0.5 truncate max-w-50 sm:max-w-xs">
                        {v.description}
                      </div>
                    )}
                  </td>
...
function ResourcesTab({ initial }: { initial: Resource[] }) {
  ...
  async function executeDelete() {
    ...
    try {
      ...
      setAlertMsg(
        "Failed to delete resource. It might be allocated to an existing booking.",
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 sm:px-6 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-semibold ${conditionColor[r.condition!] ?? conditionColor.Good}`}
                    >
                      {r.condition}
                    </span>
                  </td>
...
function UsersTab({ initial }: { initial: Profile[] }) {
  ...
  async function updateRole(userId: string, newRoleId: string) {
    ...
    try {
      ...
      if (error) throw error;
      setUsers((p) =>
        p.map((u) => (u.id === data.id ? (data as unknown as Profile) : u)),
      );
    } catch (err) {
      console.log(err);
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 mb-4 w-full">
        <div className="relative flex-1 w-full sm:w-auto min-w-50">
          <input
            value={search}
                ...
                <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                  <div className="flex-1 min-w-0 w-full">
                    ...
                    <div className="flex items-center gap-3 mb-2 flex-wrap w-full">
                      <h3 className="font-display font-bold text-slate-900 text-base wrap-break-word min-w-0">
                        {bk.title}
                      </h3>
                    ...
                    {bk.description && (
                      <p className="text-sm text-slate-500 mb-3 line-clamp-1 w-full wrap-break-word">
                        {bk.description}
                      </p>
                    )}
                    ...
                    {bk.status === "rejected" && bk.rejection_reason && (
                      <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2 mb-2 w-full wrap-break-word">
...
function FormModal({
  ...
  return (
    <div
      className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 p-4 sm:p-6 w-full"
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-4 sm:p-6 animate-slide-up max-h-full overflow-y-auto">
        <div className="flex items-center justify-between mb-5 sm:mb-6">
          <h3 className="font-display font-bold text-slate-900 text-lg sm:text-xl wrap-break-word">
            {title}
          </h3>
          <button
            onClick={onClose}
